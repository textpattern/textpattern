	...
	function preText($s,$prefs)
	{
		...
		if (!$out['id'] && !$out['s'] && !(txpinterface=='css') &&! ( txpinterface=='admin') ) {
			...
			if (strlen($u1)) {
				...
				if(!$out['s'] && !$out['c'] && !$out['q'] && !$out['author']) {
...
// -------------------------------------------------------------
	function doArticles($atts, $iscustom, $thing = NULL)
	{
		...
		if (!$iscustom)
		{
			...
			$theAtts['frontpage'] = ($s && $s=='default' && $s!='home')? true: false; //todo try to set just home section to frantpage
			$theAtts['excerpted'] = '';
		}
		extract($theAtts);
