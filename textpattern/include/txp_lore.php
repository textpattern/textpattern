...
if ($event == 'lore') {
    ...
    $available_steps = array(
        'lore_list'          => false,
        'lore_change_pageby' => true,
        'lore_multi_edit'    => true,
    );
...
function lore_list($message = '')
{
    ...
    if ($sort === '') {
        $sort = get_pref('lore_sort_column', 'time');
    } else {
        if (!in_array($sort, array('page', 'refer', 'method', 'status'))) {
            $sort = 'time';
        }

        set_pref('lore_sort_column', $sort, 'lore', PREF_HIDDEN, '', 0, PREF_PRIVATE);
    }

    if ($dir === '') {
        $dir = get_pref('lore_sort_dir', 'desc');
    } else {
        $dir = ($dir == 'asc') ? "asc" : "desc";
        set_pref('lore_sort_dir', $dir, 'lore', PREF_HIDDEN, '', 0, PREF_PRIVATE);
    }
...
function lore_multiedit_form($page, $sort, $dir, $crit, $search_method)
{
    $methods = array('delete' => gTxt('delete'));

    return multi_edit($methods, 'lore', 'lore_multi_edit', $page, $sort, $dir, $crit, $search_method);
}
