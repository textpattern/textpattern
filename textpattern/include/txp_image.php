...
class ImagePanel
{
    ...
    /**
     * List of defined image categories.
     *
     * @var array
     */

    protected $all_image_cats;

    /**
     * List of image authors who can edit content.
     *
     * @var array
     */
    ...
    protected $availableSteps = array(
        'image_list'          => false,
        'image_edit'          => false,
        'image_insert'        => true,
        'image_replace'       => true,
    ...
    public function image_list($message = '')
    {
        ...
        if ($total < 1) {
            ...
            if ($rs && numRows($rs)) {
                ...
                while ($a = nextRow($rs)) {
                    ...
                    $validator->setConstraints(array(new Validator\CategoryConstraint($category, array('type' => 'image'))));
                ...
                $contentBlock .= n.tag_end('tbody').
                    ...
                    $this->image_multiedit_form($page, $sort, $dir, $crit, $search_method).
                    tInput().
                    n.tag_end('form');
            }
        }
    ...
    public function image_multiedit_form($page, $sort, $dir, $crit, $search_method)
    {
        $categories = $this->all_image_cats ? treeSelectInput('category', $this->all_image_cats, '') : '';
        $authors = $this->all_image_authors ? selectInput('author', $this->all_image_authors, '', true) : '';
    ...
    public function image_multi_edit()
    {
        global $txp_user;
        ...
        foreach ($this->all_image_cats as $row) {
            $categories[] = $row['name'];
        }
        ...
        switch ($method) {
            case 'delete':
                return $this->image_delete($selected);
                break;
            case 'changecategory':
                $val = ps('category');
            ...
            case 'changeauthor':
                $val = ps('author');
                if (has_privs('image.edit') && isset($this->all_image_authors[$val])) {
                    $key = 'author';
                }
                break;
            default:
        ...
        if ($changed) {
            ...
            return $this->image_list(gTxt('image_updated', array('{name}' => join(', ', $changed))));
        }
    ...
    public function image_edit($message = '', $id = '')
    {
        global $file_max_upload_size, $txp_user, $event, $img_dir;
        ...
        if ($rs) {
            ...
            if ($ext != '.swf') {
                ...
                $img_info = $id.$ext.' ('.$w.' &#215; '.$h.')';
                $imgsrc = imagesrcurl($id, $ext);
                $img = '<div class="fullsize-image"><img class="content-image" src="'.$imgsrc."?$uDate".'" alt="'.$img_info.'" title="'.$img_info.'" /></div>';
            } else {
                $img = $aspect = '';
            }
            ...
            if ($thumbnail and ($ext != '.swf')) {
                ...
                $thumbsrc = imagesrcurl($id, $ext, true);
  //              $thumbobj = $this->manager->make($imgsrc);
                $thumb = '<img class="content-image" src="'.$thumbsrc."?$uDate".'" alt="'.$thumb_info.'" title="'.$thumb_info.'" />';
            } else {
                $thumb = '';
            ...
            $constraints = array(
                'float'     => Txp::get('\Textpattern\Validator\RangeConstraint', null, array(
                    'min' => 0.25, 'max' => 5, 'step' => 0.01)
                ),
                'percent'   => Txp::get('\Textpattern\Validator\RangeConstraint', null, array(
                    'min' => 0, 'max' => 100)
                ),
                'plusminus' => Txp::get('\Textpattern\Validator\RangeConstraint', null, array(
                    'min' => -100, 'max' => 100)
                ),
            );
            ...
            $buttons = array(
                'crop' => array(
                    'class' => 'ui-icon ui-extra-icon-crop',
                ),
                'rotate_l' => array(
                    'class' => 'ui-icon ui-icon-arrowreturn-1-s',
                    'val'   => '+90',
                    'name'  => 'rotate',
                ),
                'rotate_r' => array(
                    'class' => 'ui-icon ui-extra-icon-arrowreturn-1-s-reversed',
                    'val'   => '-90',
                    'name'  => 'rotate',
                ),
                'flip_h' => array(
                    'class' => 'ui-icon ui-extra-icon-flip-horizontal',
                    'val'   => 'h',
                ),
                'flip_v' => array(
            ...
            if ($can_edit) {
                // Let plugins alter filters, buttons and constraints.
                callback_event_ref('image_ui', 'constraints', 0, $rs, $constraints);
                callback_event_ref('image_ui', 'buttons', 0, $rs, $buttons);
                callback_event_ref('image_ui', 'filters', 0, $rs, $filters);

                $buttonBlock = array();
                $filterBlock = array();

                foreach ($buttons as $ctrl => $opts) {
                    $atts['id'] = 'img_'.$ctrl;
                    $atts['name'] = isset($opts['name']) ? $opts['name'] : $ctrl;
                    $atts['value'] = isset($opts['val']) ? $opts['val'] : null;
                    $buttonBlock[] = Txp::get('\Textpattern\UI\Tag', 'button')
                        ->setAtts($atts)
                        ->setContent(Txp::get('\Textpattern\UI\Span')->setAtt('class', $opts['class'])->render('complete'))
                        ->setContent(n.gTxt('img_'.$ctrl), true);
                }

                foreach ($filters as $ctrl => $opts) {
                    $ui = array();

                    switch ($opts['type']) {
                        case 'rangespin':
                            $ui[] = Txp::get('\Textpattern\UI\Number', $ctrl, (isset($opts['default']) ? $opts['default'] : 0))
                                ->setConstraints($constraints[$opts['constraints']])
                                ->setAtts(array(
                                    'id'    => 'img_'.$ctrl.'_range',
                                    'type'  => 'range',
                                    'class' => 'slider',
                            ...
                            break;
                        case 'radioset':
                            $ui[] = Txp::get('\Textpattern\UI\RadioSet', $opts['name'], $opts['options'], $opts['default']);
                            break;
                    }
            ...
            $imageBlock[] = ($can_upload
                ? pluggable_ui(
                    'image_ui',
                    'image_edit',
                    upload_form('replace_image', 'replace_image_form', 'image_replace', 'image', $id, $file_max_upload_size, 'image-upload', ' image-replace', array('div', 'div'), '' , implode(',', $this->extensions)),
                    $rs)
                : ''
            );
            ...
            echo n.'<div class="txp-layout">'.
                ...
                n.tag(
                    tag(implode(n, $createBlock), 'div', array('class' => 'txp-layout-cell-row txp-list-head')).
            ...
            $this->image_list(array(gTxt('unknown_image'), E_ERROR));
        }
    }
    ...
    public function image_insert()
    {
        ...
        if (is_array($img_result)) {
            ...
            return $this->image_list(array($img_result, E_ERROR));
        }
    }
    ...
    public function image_replace()
    {
        ...
        if (is_array($img_result)) {
            ...
            return $this->image_edit(array($img_result, E_ERROR), $id);
        }
    }
    ...
    public function thumbnail_insert()
    {
        global $txp_user;
        ...
        if (empty($file)) {
            $this->image_edit(array(upload_get_errormsg(UPLOAD_ERR_NO_FILE), E_ERROR), $id);
        ...
        if (($file !== false) && !empty($this->extensions[$extension])) {
            $ext = $this->extensions[$extension];
            $newpath = IMPATH.$id.'t'.$ext;

            if (shift_uploaded_file($file, $newpath) == false) {
                $this->image_edit(array(gTxt('directory_permissions', array('{path}' => $newpath)), E_ERROR), $id);
            } else {
                chmod($newpath, 0644);
                ...
                $this->image_edit($message, $id);
            }
        } else {
            if ($file === false) {
                $this->image_edit(array(upload_get_errormsg($_FILES['thefile']['error']), E_ERROR), $id);
            } else {
                $this->image_edit(array(gTxt('only_graphic_files_allowed', array('{formats}' => join(', ', $this->extensions))), E_ERROR), $id);
            }
        }
    }
    ...
    public function image_delete($ids = array())
    {
    ...
    public function image_change_pageby()
    {
    ...
    public function thumbnail_create()
    {
        ...
        if ($width === '' && $height === '') {
            $this->image_edit(array(gTxt('invalid_width_or_height'), E_ERROR), $id);
    ...
    public function thumbnail_delete()
    {
        global $txp_user;
        ...
        if ($t->delete()) {
            ...
            $this->image_edit(gTxt('thumbnail_deleted'), $id);
        } else {
            $this->image_edit(array(gTxt('thumbnail_delete_failed'), E_ERROR), $id);
        }
    }

    /**
     * Perform an image transformation via Ajax request.
     *
     * Crop, rotate, flip, greyscale, tint, etc.
     *
     * @param array  $transform [Ajax] Name of the transform to apply
     * @param array  $val       [Ajax] Value to pass to the transform
     * @param string $name      [Ajax] File name/path to operate upon. If omitted, uses full-size source
     * @param string $id        [Ajax] File id being operated upon
     * @param string $ext       [Ajax] File extension of file being operated upon
     */

    public function transform()
    {
        // @todo Use when new path pref when available.
        ...
        array_multisort($availableTransforms, SORT_NUMERIC, array_column($availableTransforms, 'order'));
