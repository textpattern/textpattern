...
function parse($thing, $condition = true, $in_tag = true)
{
    ...
    if (!isset($thing) || !$short_tags && false === strpos($thing, '<txp:') ||
        $short_tags && strlen($thing) >= TEXTPATTERN_HASH_LENGTH && !preg_match('@<(?:'.TXP_PATTERN.'):@', $thing)) {
    ...
    if (!empty($txp_atts['not']) && $in_tag && (string)$txp_atts['not'] === $hash) {
...
function processTags($tag, $atts = '', $thing = null, $log = false)
{
    ...
    if ($atts) {
        ...
        if (!empty($txp_atts['not']) && isset($thing)) {
            $txp_atts['not'] = txp_hash($thing);
        }
    } else {
        $txp_atts = null;
        $split = array();
