        ...
        if (is_callable($c)) {
            if ($production_status !== 'live') {
                $trace->start("\t[Call function: '".Txp::get('\Textpattern\Type\TypeCallable', $c)->toString()."'".
                    (empty($argv) || !is_serializable($argv) ? '' : ", argv='".serialize($argv)."'")."]");
            }
...
/**
 * Polyfill for PHP <7 to determine if the given variable is iterable
 *
...
/**
 * Determine if the given variable is serializable
 *
 * @param  mixed $var     The variable to test
 * @param  bool  $iterate Whether to iterate through $var and check elements for serializability
 * @since  4.9.0
 * @return bool
 */
